...
public class Chakatons : ControllerBase
{
    ...
    public IActionResult Create([FromBody] ChakatonDto chakaton)
    {
        try
        {
            ...
            bool access = _services.AuthService.CheckAccess(Convert.ToInt32(jwtUid));
            if (!access)
            {
                return StatusCode(401, ResponseManager.SendError("Access forbidden"));
            }
    ...
    // GET: api/chakatons/AmountStats
    /// <summary>
    /// Получение статистики по тегам популярности.
    /// </summary>
    /// <remarks>
    /// Возвращает полученные данные всех хакатонов.
    /// </remarks>
    ...
    public IActionResult GetStats()
    {
        try
        {
            List<ChakatonStatAmount> result = _services.ChakatonService.GetStatsAmount();
            return Ok(ResponseManager.SendJson(result));
        }
        catch (Exception e)
        {
